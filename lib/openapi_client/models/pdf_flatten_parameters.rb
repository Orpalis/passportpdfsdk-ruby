  ...
  class PdfFlattenParameters
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PdfFlattenParameters` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PdfFlattenParameters`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'flatten_annotations')
        self.flatten_annotations = attributes[:'flatten_annotations']
      else
        self.flatten_annotations = false
      end

      if attributes.key?(:'flatten_form_fields')
        self.flatten_form_fields = attributes[:'flatten_form_fields']
      else
        self.flatten_form_fields = false
      end

      if attributes.key?(:'flatten_layers')
        self.flatten_layers = attributes[:'flatten_layers']
      else
